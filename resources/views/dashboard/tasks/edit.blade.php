...
@section('content')
    ...
    <div class="body_scroll">
            ...
            <div class="row">
                <div class="col-lg-7 col-md-6 col-sm-12">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{adminUrl('groups/' . $event->id)}}">المهام </a></li>
        ...
        <div class="container-fluid">
            ...
            <!-- Vertical Layout -->
            <form action="{{adminUrl('tasks/update/'. $task->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('patch')
                                ...
                                <div class="row">
                                    <div class="col-lg-12 col-md-12 col-sm-3">
                                        ...
                                        <div class="form-group">
                                            <input required type="text" name="name" value="{{$task->task_title}}" id="email_address" class="form-control" placeholder="ادخل اسم المهمة">
                                        </div>
                                    </div>
                                        ...
                                        <select name="users[]" class="form-control show-tick ms select2"  multiple data-placeholder="اختر اعضاء المهمة من بين المستخدمين">
                                            @if($users)
                                                @foreach($users as $user)
                                                    ...
                                                    @foreach($task->members as $task_user)
                                                        {{$task_user->id == $user->id ? 'selected' : ''}}
                                                    @endforeach
                                                    >
                                                        {{$user->name}}
                                                    </option>
