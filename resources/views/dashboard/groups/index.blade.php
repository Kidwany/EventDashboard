                                ...
                                <table id="ssss" class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    ...
                                    @if($groups)
                                        @foreach($groups as $group)
                                            <tr>
                                                <td>{{$group->id}}</td>
                                                <td><div style="width: 25px; height: 25px; background: {{$group->color_id ? $group->color->hex : '#000'}}; border-radius: 50%"></div></td>
                                                <td>{{$group->name}}</td>
                                                <td>{{$group->users->count()}}</td>
                                                <td>{{$group->created_by ? $group->createdBy->name : ''}}</td>
                                                <td>{{$group->created_at->diffForHumans()}}</td>
                                                <td style="display: flex">
                                                    <a href="{{adminUrl('groups/edit/' . $event->id . '/' . $group->id)}}" class="btn btn-primary btn-sm"><i class="zmdi zmdi-edit"></i> </a>
            ...
            <div class="modal fade" id="delete{{$group->id}}" tabindex="-1" role="dialog">
                <div class="modal-dialog" role="document">
                    <div class="modal-content bg-red">
                        ...
                        <form id="deleteUser{{$group->id}}" style="display: none" action="{{url('groups/delete/' . $group->id)}}" method="post">
                            @csrf
                            @method('delete')
                        </form>
