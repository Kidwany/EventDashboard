                                    ...
                                    @if($users)
                                        @foreach($users as $user)
                                            <tr>
                                                ...
                                                <td>{{$user->name}}</td>
                                                <td>{{$user->phone}}</td>
                                                <td>{{$user->company->category->category_ar}}</td>
                                                <td>{{$user->created_at->diffForHumans()}}</td>
                                                <td style="display: flex">
                                                    <a href="{{adminUrl('user/' . $user->id . '/edit')}}" class="btn btn-primary btn-sm"><i class="zmdi zmdi-edit"></i> </a>
            ...
            <div class="modal fade" id="delete{{$user->id}}" tabindex="-1" role="dialog">
                <div class="modal-dialog" role="document">
                    <div class="modal-content bg-red">
                        ...
                        <form id="deleteUser{{$user->id}}" style="display: none" action="{{url('user/' . $user->id . '/delete')}}" method="post">
                            @csrf
                            @method('delete')
                        </form>
