...
@section('content')
    ...
    <div class="body_scroll">
                    ...
                    <div class="card">
                        <div class="body">
                            ...
                            <ul class="nav nav-tabs p-0 mb-3 nav-tabs-success" role="tablist">
                                ...
                                <li class="nav-item"><a class="nav-link {{Request::is('expected_expenses/*') ? 'active' : ''}}"      href="{{adminUrl('finance/' . $event->id)}}"><i class="zmdi zmdi-money"></i> البيانات المالية </a></li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div class="container-fluid">
            ...
            <!-- Vertical Layout -->
                <form action="{{url('expected_expenses/' . $expense->id . '/update')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                                ...
                                <div class="row" >
                                        ...
                                        <select required name="category_id" class="form-control show-tick ms select2"  data-placeholder="اختر الخدمات التي تقدمها المغسلة">
                                            @if($categories)
                                                @foreach($categories as $category)
                                                    <option value="{{$category->id}}" {{$expense->category_id == $category->id ? 'selected' : ''}}>{{$category->category}}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                                    <div class="col-lg-6 col-md-12 col-sm-3">
                                        ...
                                        <div class="form-group d-flex justify-content-between">
                                            <input required type="text" value="{{$expense->item}}" name="item" id="email_address" class="form-control" placeholder="ادخل بند التكلفة">
                                        </div>
                                    </div>
                                    <div class="col-lg-6 col-md-12 col-sm-3">
                                        <label for="email_address">التكلفة المتوقعة (ر.س)</label>
                                        <div class="form-group d-flex justify-content-between">
                                            <input step="any" type="number" value="{{$expense->expected_value}}" name="expected" id="email_address" class="form-control" placeholder="ادخل التكلفة المتوقعة">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-lg-6 col-md-12 col-sm-3">
                                        ...
                                        <div class="form-group d-flex justify-content-between">
                                            <input step="any" type="number" value="{{$expense->real_value}}" name="real" id="email_address" class="form-control" placeholder="ادخل التكلفة الفعلية">
                                        </div>
                                    </div>
                                </div>
